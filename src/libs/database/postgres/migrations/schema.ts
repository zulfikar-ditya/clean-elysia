import { pgTable, varchar, timestamp, text, integer, uniqueIndex, foreignKey, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const userStatus = pgEnum("UserStatus", ['ACTIVE', 'INACTIVE', 'BLOCKED'])
...
export const userEmailVerification = pgTable("UserEmailVerification", {
	id: text().primaryKey().notNull(),
	userId: text().notNull(),
...
export const userRole = pgTable("UserRole", {
	...
	primaryKey({ columns: [table.userId, table.roleId], name: "UserRole_pkey"}),
]);
