...
import { DateToolkit, verificationTokenLifetime } from "@packages/*";
...
export const AuthService = {
	...
	async resentVerificationEmail(
		...
		await db.transaction(async (trx) => {
			await trx
				...
				.where(eq(email_verificationsTable.user_id, user.id));
	...
	async verifyEmail(token: string, ipAddress: string, userAgent: string) {
		...
		await db.transaction(async (trx) => {
			...
			await trx
				.delete(email_verificationsTable)
				.where(eq(email_verificationsTable.id, emailVerification[0].id));
		});
	},
